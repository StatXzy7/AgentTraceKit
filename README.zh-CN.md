...
使用 `atk browse` 打开可视化 session 选择器，点击 **Collect & Review** 即可采集并打开报告。使用 `atk view BUNDLE` 打开增强版离线 Viewer，使用 `atk annotate BUNDLE` 进入本地标注工作台。标注保存在 `annotation/annotations.jsonl`，并通过稳定的 event ID 和原始行号回溯证据。多个 bundle 可用 `atk corpus index DIRECTORY` 建立本地可重建索引。
